...
-- 데모 워크플로 시드. 014_embed_widgets 와 동일한 가드 패턴:
-- 시드 소유 계정이 없는 환경(프로덕션)에서는 아무것도 삽입하지 않고 조용히 종료한다.
-- 가드 없이 (SELECT id FROM users ...) 를 owner_id 에 직접 넣으면 계정 부재 시
-- NOT NULL 위반으로 파일 전체가 롤백되어 workflows 테이블 자체가 생성되지 않는다.
DO $$
DECLARE
  seed_owner UUID;
BEGIN
  SELECT id INTO seed_owner FROM users WHERE email='1@openagentx.org' LIMIT 1;
