...
function loadSdk(clientId: string, currency: string): Promise<void> {
  ...
  sdkPromise = new Promise<void>((resolve, reject) => {
    ...
    s.async = true;
    s.onload = () => resolve();
    s.onerror = () => { sdkPromise = null; reject(new Error('PayPal SDK 로드 실패')); };
...
export default function PayPalButton({ amount, currency = 'USD', description, jobId, onSuccess, onError, onCancel, disabled = false }: PayPalButtonProps) {
  ...
  const api = useCallback(async (body: Record<string, unknown>) => {
    ...
    if (!res.ok || json.success === false) throw new Error(json.error ?? '결제 처리 중 오류가 발생했습니다');
    return json;
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!paypal?.Buttons) throw new Error('PayPal SDK가 올바르게 로드되지 않았습니다');

        const buttons = paypal.Buttons({
          style: { layout: 'vertical', color: 'blue', shape: 'rect', label: 'paypal', height: 45 },
          createOrder: async () => {
            const r = await api({ action: 'create-order', amount, currency, description: description ?? 'OpenAgentX 결제', job_id: jobId });
          ...
          onApprove: async (data) => {
            ...
            } catch (e) { const m = e instanceof Error ? e.message : '결제 캡처 실패'; setError(m); onError?.(m); }
          },
          onCancel: () => onCancel?.(),
          onError: (err: unknown) => { const m = err instanceof Error ? err.message : 'PayPal 결제 오류'; setError(m); onError?.(m); },
        });
        if (cancelled || !ref.current) return;
        ...
        if (!cancelled) { setError(e instanceof Error ? e.message : 'PayPal 초기화 실패'); setLoading(false); }
      }
    })();
    return () => { cancelled = true; };
  }, [amount, currency, description, jobId, disabled, api, onSuccess, onError, onCancel]);

  if (disabled) return <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 text-center text-sm text-gray-500">결제를 진행할 수 없습니다</div>;
    ...
    <div className="w-full">
      {loading && !error && (
        <div className="flex items-center justify-center rounded-lg border border-gray-200 bg-gray-50 p-4">
          ...
          <span className="text-sm text-gray-600">PayPal 로드 중...</span>
        </div>
      )}
